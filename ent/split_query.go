...
func (sq *SplitQuery) All(ctx context.Context) ([]*Split, error) {
	...
	return withInterceptors[[]*Split](ctx, sq, qr, sq.inters)
}
...
func (sq *SplitQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, sq, querierCount[*SplitQuery](), sq.inters)
}
...
func (sq *SplitQuery) prepareQuery(ctx context.Context) error {
...
func (sq *SplitQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Split, error) {
	var (
		nodes       = []*Split{}
		withFKs     = sq.withFKs
		_spec       = sq.querySpec()
	...
	if query := sq.withStock; query != nil {
		if err := sq.loadStock(ctx, query, nodes, nil,
			func(n *Split, e *Entity) { n.Edges.Stock = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SplitQuery) loadStock(ctx context.Context, query *EntityQuery, nodes []*Split, init func(*Split), assign func(*Split, *Entity)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Split)
	for i := range nodes {
		if nodes[i].entity_splits == nil {
			continue
		}
		fk := *nodes[i].entity_splits
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "entity_splits" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sgb *SplitGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SplitQuery, *SplitGroupBy](ctx, sgb.build, sgb, sgb.build.inters, v)
}

func (sgb *SplitGroupBy) sqlScan(ctx context.Context, root *SplitQuery, v any) error {
...
func (ss *SplitSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SplitQuery, *SplitSelect](ctx, ss.SplitQuery, ss, ss.inters, v)
}

func (ss *SplitSelect) sqlScan(ctx context.Context, root *SplitQuery, v any) error {
